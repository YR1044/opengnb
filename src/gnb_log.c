...
static void log_udp_output(gnb_log_ctx_t *log, uint8_t log_type, char *log_string, int log_string_len){
    ...
    if ( log->socket6_fd > 0 ) {
        ...
        sendto(log->socket6_fd, log_string, log_string_len, 0, (struct sockaddr *)&saddr6, sizeof(struct sockaddr_in6));
    }
    if ( log->socket4_fd > 0 ) {
        ...
        sendto(log->socket4_fd, log_string, log_string_len, 0, (struct sockaddr *)&saddr4, sizeof(struct sockaddr_in));
    }
}
...
static void log_udp_binary_output(gnb_log_ctx_t *log, uint8_t log_type, uint8_t log_id, char *log_string_buffer, int log_string_len){
    ...
    if ( log->socket4_fd > 0 ) {
        ...
        saddr4.sin_port   = log->port4;
        memcpy(&saddr4.sin_addr.s_addr, log->addr4, 4);
        sendto(log->socket4_fd, (void *)gnb_payload16, data_size+4, 0, (struct sockaddr *)&saddr4, sizeof(struct sockaddr_in));
    }
}
...
void gnb_logf(gnb_log_ctx_t *log, uint8_t log_type, uint8_t log_id, uint8_t level, const char *format, ...){
    ...
    if ( (log->output_type & GNB_LOG_OUTPUT_STDOUT) && (log->config_table[log_id].console_level >= level) ) {
        log_console_output(log_type, log_string, log_string_len);
    }
    if ( log->output_type & GNB_LOG_OUTPUT_FILE && (log->config_table[log_id].file_level >= level) ) {
        log_file_output(log, log_type, log_string, log_string_len);
    }
    if ( log->output_type & GNB_LOG_OUTPUT_UDP && (log->config_table[log_id].udp_level >= level) ) {
